package com.example.wushendemo.metrics;
...
public class MetricsAspect {
    ...
    @Pointcut("@within(com.example.wushendemo.metrics.Metrics)")
    public void metricsForClass() {}

    @Pointcut("@annotation(com.example.wushendemo.metrics.Metrics)")
    public void metricsForMethod() {}

    @Around("metricsForClass() || metricsForMethod())")
    public Object metrics(ProceedingJoinPoint pjp) throws Throwable {
        ...
        String.format(
                "【%s】【%s】",
                signature.getDeclaringType().toString(), signature.toLongString());*/
        ...
        if (metrics.logParameters()) {
            log.info(
                    String.format(
                            "【入参日志】调用 %s 的参数是：【%s】",
                            name, objectMapper.writeValueAsString(pjp.getArgs())));
        }
